...
ApplicationUI::ApplicationUI(bb::cascades::Application *app) :
	...
	bool res_changed = connect(devSupport, SIGNAL(assetsChanged(QUrl)), this,
			SLOT(reloadQML(QUrl)));
	Q_ASSERT(res_changed);
	Q_UNUSED(res_changed);

	bool res_cleanup = connect(Application::instance(), SIGNAL(aboutToQuit()),
			this, SLOT(cleanup()));
...
void ApplicationUI::onSystemLanguageChanged() {
	...
	QString file_name = QString("QMLUpdateTest_%1").arg(locale_string);
